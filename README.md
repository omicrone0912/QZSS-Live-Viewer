...
3. **軌道データ（TLE）の取得ロジック実装 (`src/api/tle.ts`)**:
   [CelesTrak](https://celestrak.org/)が公開している最新の衛星軌道データ（TLE形式）を、北アメリカ航空宇宙防衛司令部（NORAD）カタログ番号（49336, 42738, 42917, 42965, 62876）を指定して直接取得するAPIクライアントを構築しました。
   また、API負荷軽減のため、数時間のキャッシュを設けています。
5. **衛星座標の計算 (`src/utils/orbit.ts`)**:
   取得したTLE文字列と現在時刻を `satellite.js` ライブラリに渡し、SGP4軌道モデルを用いて緯度・経度・高度・速度を算出する関数を使用しています。
6. **UIとアニメーションの連携 (`src/main.ts`)**:
